...
def main():
	...
	fontObjectMN = pygame.font.Font('freesansbold.ttf',90)
	...
	initStaticText = []
	#HEADER Message
	MachineTag_SO = fontObjectHeader.render("___PQMFG Line Controller___",False, LIGHT_BLUE)
	...
	#Main screen buttons
	buttonChangeWO = pygbutton.PygButton((WINDOWWIDTH-2*BRD_SPACER-BUTTON_WIDTH, 2*BRD_SPACER, BUTTON_WIDTH, BUTTON_HEIGHT), 'Change Work Order')
	buttonChangeWO.font = fontObjectDefault

	buttonCompleteWO = pygbutton.PygButton((WINDOWWIDTH-2*BRD_SPACER-BUTTON_WIDTH, 3*BRD_SPACER+BUTTON_HEIGHT, BUTTON_WIDTH, BUTTON_HEIGHT), 'Complete Current Work Order')
	buttonCompleteWO.font = fontObjectDefault
	...
	buttonMachineDown = pygbutton.PygButton((WINDOWWIDTH-2*BRD_SPACER-BUTTON_WIDTH, 8*BRD_SPACER+4*BUTTON_HEIGHT, BUTTON_WIDTH, BUTTON_HEIGHT), 'Bring Line Down')
	buttonMachineDown.font = fontObjectDefault

	buttonMachineUp = pygbutton.PygButton((WINDOWWIDTH-2*BRD_SPACER-BUTTON_WIDTH, 9*BRD_SPACER+5*BUTTON_HEIGHT, BUTTON_WIDTH, BUTTON_HEIGHT), 'Bring Line Up')
	...
	buttonCancle = pygbutton.PygButton((WINDOWWIDTH/2+BRD_SPACER, WINDOWHEIGHT-BUTTON_HEIGHT- 2*BRD_SPACER, BUTTON_WIDTH, BUTTON_HEIGHT), 'Cancle')
	...
	dwnTimeButtons['Mid_Cancle'] = pygbutton.PygButton((WINDOWWIDTH/2-BUTTON_WIDTH/2, WINDOWHEIGHT/2+int(2.5*BUTTON_HEIGHT)+2*BRD_SPACER, BUTTON_WIDTH, BUTTON_HEIGHT), 'Cancle')
	...
	while True: # main game loop
		...
		for event in pygame.event.get(): # event handling loop
			...
			if not scanVal == '' and not scanVal.startswith('#'):
				...
				if scanVal == '#MAINTENANCE':
					cur_AL.changeState('Maitenance')
					GUI_STATE = 0
				elif scanVal == '#INVENTORY':
					cur_AL.changeState('Inventory')
					GUI_STATE = 0
				elif scanVal == '#QUALITY_CONTROL':
					cur_AL.changeState('Quality_Control')
					GUI_STATE = 0
				elif scanVal == '#CANCEL': 
					GUI_STATE = 0
			...
			for key in numPadDic.keys():
				if 'click' in numPadDic[key].handleEvent(event):
					...
					if displayText == [] and not key == '&' and not key == 'DEL' and not key == '+/-':
			...
			if 'click' in buttonConfirmEvent or scanVal == '#CONFIRM':
				...
				if GUI_STATE  ==2 and not displayText==[] and not cur_AL.getCurrentState()[0]==displayText[0]:
					...
					if numPadDic['+/-'].bgcolor == GREEN:
						cur_AL.inc_CurPassCount(amount = int(displayText[0]), force = True)
					else:
						cur_AL.inc_CurFailCount(amount = int(displayText[0]), force = True)
					GUI_STATE = 7.1
					scanVal = ''
		...
		if not GUI_STATE == 0:
			...
			if GUI_STATE == 2 or GUI_STATE ==3 or GUI_STATE ==7:
				...
				#-------------------------------------------------------
				for key in numPadDic.keys():
					if (GUI_STATE == 2 or GUI_STATE == 7) and key =='&':
						pass
				...
				if GUI_STATE == 2:
					Header_SO = pygame.font.Font('freesansbold.ttf',36).render("Please Type Or Scan New WO#",False, WHITE)
			...
			#If in bring machine up or down state
			elif GUI_STATE == 5 or GUI_STATE == 6:
				...
				if GUI_STATE == 5:
					...
					Header_SO2 = fontObjectHeader.render("Is machine currently running?",False, WHITE)
					Header_Rect2 = Header_SO2.get_rect()
					Header_Rect2.topleft = (WINDOWWIDTH/2 - Header_Rect2[2]/2,BRD_SPACER*5 +Header_Rect[3])

					Header_SO3 = fontObjectHeader.render("Was down for: "+cur_AL.getCurrentState()[2],False, BLACK)
			...
			#Heres where we draw all the mid state pannels
				...
				if GUI_STATE == 2.1:
					...
					if GUI_STATE == 3.1:
						...
						Header_SO2 = fontObjectHeader.render('You Have '+('Added To' if numPadDic['+/-'].bgcolor == GREEN else 'Subtracted From')+' Pass log',False, WHITE)
			...
			#Draw Top Left Box
			pygame.draw.rect(DISPLAYSURFACE, GREY, (BRD_SPACER,BRD_SPACER, IDSIZE + BRD_SPACER, ((WINDOWHEIGHT-(2*BRD_SPACER))/5)))
			pygame.draw.rect(DISPLAYSURFACE, BLACK, (BRD_SPACER,BRD_SPACER, IDSIZE, ((WINDOWHEIGHT-(2*BRD_SPACER))/5)-BRD_SPACER))
			...
			MachineID_Rect.topleft = (BRD_SPACER,BRD_SPACER)
